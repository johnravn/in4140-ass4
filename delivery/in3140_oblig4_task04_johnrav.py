...
class PID:
    ...
    def __call__(self, desired_theta, current_theta, velocity_theta, dt):
        ...
        # Use the gains as "mode selectors": if c!=0 -> PIDD, else if i!=0 -> PID,
        # else if d!=0 -> PD, else P. This way sliders/params work without changing code.
        if self.c != 0.0:
    ...
    def PD_ctrl(self, desired_theta, current_theta, velocity_theta, dt):
        ...
        _ = dt  # dt not needed when using measured velocity as derivative term
        self.error = desired_theta - current_theta
        # Derivative on measurement (avoids kick): d/dt(error) ≈ -velocity
        return (self.p * self.error) - (self.d * velocity_theta)
    ...
    def PID_ctrl(self, desired_theta, current_theta, velocity_theta, dt):
        ...
        return (self.p * self.error) + (self.i * self.integral) - (self.d * velocity_theta)
    ...
    def PIDD_ctrl(self, desired_theta, current_theta, velocity_theta, dt):
        ...
        self.error = desired_theta - current_theta
        self.integral += self.error * dt
        # Simple nonlinearity that "pushes" harder for larger errors but saturates smoothly.
        nonlinear = math.tanh(self.error)
        return (
            (self.p * self.error)
            + (self.i * self.integral)
            - (self.d * velocity_theta)
            + (self.c * nonlinear)
        )
